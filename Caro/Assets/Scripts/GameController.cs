...
public class GameController : MonoBehaviour {
	...
	void Update () {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                if (status) {
                    Instantiate(X, hit.collider.gameObject.transform.position, Quaternion.identity);
				} else {
                    Instantiate(O, hit.collider.gameObject.transform.position, Quaternion.identity);
				}
				status = ! status;
